...
class AuthViewModel: ObservableObject {
    ...
    func signInWithApple(
        ...
        do {
            ...
            signedInResult = SignInResult(
                accessToken: tokenResponse.accessToken,
                refreshToken: tokenResponse.refreshToken,
                userId: userIdentifier,
                isNewUser: email != nil   // Apple only returns email on first sign-in
            )
        } catch {
    ...
    /// Calls POST /auth/dev-login — backend creates a stable test user and returns
    /// a real JWT pair without any OAuth flow. Only works when backend is in
    /// development mode (environment=development in .env).
    func devLogin() async {
        state = .loading
        errorMessage = nil
